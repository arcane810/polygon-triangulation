...
DCEL monotoneTriangulation(DCEL &dcel) {
    ...
    for (auto it : dcel.edges) {
        new_edges.push_back({it.v1->index, it.v2->index});
    }
...
void faceTriangulation(Face &face, DCEL &dcel,
    ...
    HalfEdge *curr = head;
    vertices.push_back({curr->origin, -1});
    curr = head->next;
    while (curr != head) {
        vertices.push_back({curr->origin, -1});
        curr = curr->next;
    ...
    for (int j = 2; j < vertices.size() - 1; j++) {
        ...
        if (dcel.vertices[ji].point.isBelow(dcel.vertices[jpi].point) !=
            ...
            while (!st.empty()) {
                ...
                diagonals.push_back({ji, index_in_dcel[tp.second]});
            }
            ...
            while (!st.empty()) {
                if (dcel.vertices[ji].point.isBelow(dcel.vertices[jpi].point)) {
                    if (!counterClockwise(vertices[j].first->point,
                                          ...
                                          st.top().first->point)) {
                        diagonals.push_back(
                            {ji, index_in_dcel[st.top().second]});
                        lpop = st.top();
                        st.pop();
                    } else {
                        break;
    ...
    while (!st.empty()) {
        diagonals.push_back(
            {vertices.back().first->index, st.top().first->index});
        st.pop();
    }
}